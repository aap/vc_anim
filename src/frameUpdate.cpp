...
FrameUpdateCallBackSkinnedWith3dVelocityExtraction(AnimBlendFrameData *frame, CAnimBlendNode **nodes)
{
	...
	if(!(frame->updateFlag & CAnimBlendAssociation::NoRotation)){
	...
	if(!(frame->updateFlag & CAnimBlendAssociation::NoTranslation)){
...
FrameUpdateCallBackSkinned(AnimBlendFrameData *frame, void *arg)
{
	...
	if (frame->updateFlag & CAnimBlendAssociation::Flag8 && gpAnimBlendClump->pedPosition){
		if(frame->updateFlag & CAnimBlendAssociation::Partial)
