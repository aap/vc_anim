...
CAnimBlendNode::CalcDeltasCompressed(void)
{
	...
	if(!(seq->flag & 1))
		return;
	RFrame *frame0 = (RFrame*)GETCFRAME(seq, this->frame0);
	RFrame *frame1 = (RFrame*)GETCFRAME(seq, this->frame1);
	...
	if(norm < 1.0f){
		frame0->rot.x = -frame0->rot.x;
		frame0->rot.y = -frame0->rot.y;
		frame0->rot.z = -frame0->rot.z;
		frame0->rot.w = -frame0->rot.w;
	}
	norm = frame0->rot.x*frame1->rot.x +
	       frame0->rot.y*frame1->rot.y +
	       frame0->rot.z*frame1->rot.z +
	       frame0->rot.w*frame1->rot.w;
	if(norm > 1.0f)
...
CAnimBlendNode::SetupKeyFrameCompressed(void)
{
	...
	if(this->sequence->numFrames == 1){
		this->frame0 = 0;
		this->time = 0.0f;
		this->CalcDeltasCompressed();
	}else{
		this->time = ((RFrame*)GETCFRAME(this->sequence, this->frame0))->time;
		this->CalcDeltasCompressed();
	}
}
